...
logging.info("Aplikacja startuje v2.5")
...
class AddRecordDialog(tb.Toplevel):
    def __init__(self, master):
        ...
        frm = tb.Frame(self, padding=14, style="Card.TFrame")
        frm.pack(fill=BOTH, expand=True)

        def row(r, label, var, width=28):
            tb.Label(frm, text=label).grid(row=r, column=0, sticky="e", padx=6, pady=4)
            e = tb.Entry(frm, textvariable=var, width=width)
        ...
        try:
            cb_g = tb.Combobox(frm, values=["koza","owca","bydło","inne"], textvariable=self.v_gatunek, state="readonly", width=26)
            cb_g.grid(row=2, column=1, sticky="w", padx=6, pady=4)
            cb_s = tb.Combobox(frm, values=["żywe","padłe","ubój","zbyte"], textvariable=self.v_status, state="readonly", width=26)
            cb_s.grid(row=12, column=1, sticky="w", padx=6, pady=4)
            cb_p = tb.Combobox(frm, values=["M","K"], textvariable=self.v_plec, state="readonly", width=6)
